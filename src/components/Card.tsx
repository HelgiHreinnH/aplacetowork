...
const Card: React.FC<CardProps> = ({ title, image, description, squareMeters, employees, usage }) => {
  ...
  return (
    <div className="relative w-full h-full" style={{ perspective: '1000px' }}>
      ...
      >
        <div className={`absolute w-full h-full backface-hidden bg-gray-100 rounded-lg shadow-md overflow-hidden ${isFlipped ? 'hidden' : 'flex flex-col'}`}>
        ...
        <div 
          className={`absolute w-full h-full backface-hidden bg-gray-100 rounded-lg shadow-md overflow-hidden ${isFlipped ? 'block' : 'hidden'}`} 
